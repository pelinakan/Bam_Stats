 ...
 * Main of app
 */
int qaCompute(string bamfilename)
{
    samfile_t *fp;
    FILE *outputFile;
    Options userOpt;
...
*/
//    std::string alignFile(bamfilename);
    fp = open_alignment_file(bamfilename.c_str());
    EXIT_IF_NULL(fp);
/*
    if (outsideHeader) {
        //Trick header!
 ...
 */
    string outputfilename = bamfilename.substr(0,(bamfilename.length() - 3));
    outputfilename.append(".coverage");
    if ((outputFile = fopen(outputfilename.c_str(), "wt")) == 0) {
